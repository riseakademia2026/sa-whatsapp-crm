...
def webhook():
    ...
    try:
        ...
        if messages:
            ...
            customer_name = phone
            if contacts:
                customer_name = contacts[0].get("profile", {}).get("name", phone)
            ...
            customer = get_or_create_customer(phone, customer_name)
            save_message(customer["id"], "in", text)

            print("NEW CUSTOMER SAVED:", phone, text)
